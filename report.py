...
class reportClass:
    def __init__(self,root):
        ...
        lbl_search = Entry(self.root,textvariable=self.var_search, font=("goudy old style", 20),bg='lightyellow').place(x=550, y=100,width=150)
        btn_search = Button(self.root, text='Search', font=("goudy old style", 15, "bold"), bg="#03a9f4", fg='white',
                            cursor='hand2',command=self.search).place(x=710, y=100, width=100, height=35)
        btn_clear = Button(self.root, text='Clear', font=("goudy old style", 15, "bold"), bg="gray", fg='white',
                            cursor='hand2',command=self.clear).place(x=830, y=100, width=100, height=35)
        ...
        lbl_marks = Label(self.root, text='Marks Obtain', font=("goudy old style", 15, "bold"), bg='white',bd=2,relief=GROOVE).place(x=600,y=230,width=150,height=50)
        ...
        btn_delete = Button(self.root, text='Delete',font=("goudy old style", 15, "bold"), bg="red", fg='white',
                           cursor='hand2',command=self.delete).place(x=520, y=350, width=150, height=35)
    ...
    def search(self):
        ...
        try:
            ...
            if self.var_search.get()=='':
                ...
                if row != None:
                    self.var_id=row[0]
                    self.roll.config(text=str(row[1]))
                    self.name.config(text=str(row[2]))
                    self.course.config(text=str(row[3]))
                    self.marks.config(text=str(row[4]))
                    self.full_marks.config(text=str(row[5]))
                    self.percentage.config(text=str(row[6]))
                else:
